    ...
    const fetchDataName = async () => {
      try {
        const resName = await axios.post(
          `https://four-gems-api-c21adc436e90.herokuapp.com/user/get-info-by-token?token=${token}`,
          {},
          {
            headers: {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await axios.get(
          `https://four-gems-api-c21adc436e90.herokuapp.com/order/get-money-by-date?countId=1&startDate=${endDate}&endDate=${endDate}`,
          {
            headers: {
              Authorization: "Bearer " + token, //the token is a variable which holds the token
            },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <H5 color="grey.600">Here’s what happening with your store today!</H5>

        {/*<H3 mt={3}>15,350.25</H3>*/}
        {/*<Paragraph color="grey.600">Today’s Visit</Paragraph>*/}

        {/*<H1 mt={1}>{currency(start)}</H1>*/}
        <H1 mt={1}>{isNaN(start) ? <SpinnerLoading /> : currency(start)}</H1>
